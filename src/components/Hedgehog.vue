...
const baseCXRight = 610;
const baseCYRight = 560;
const baseCXLeft = 430;
const baseCYLeft = 560;

const isBlinking = ref(false);
let blinkingEyeRight = null;
let blinkingEyeLeft = null;
const svgRef = ref(null);
let blinkTimeout = null;

const updateEye = (eye, baseCX, baseCY, clientX, clientY) => {
  if (!eye) return;
  ...
  const distance = Math.min(Math.sqrt(dx ** 2 + dy ** 2), 40);
...
const handleMove = (event) => {
  updateEye(blinkingEyeRight, baseCXRight, baseCYRight, event.clientX, event.clientY);
  updateEye(blinkingEyeLeft, baseCXLeft, baseCYLeft, event.clientX, event.clientY);
};

onMounted(() => {
  if (svgRef.value) {
    blinkingEyeRight = svgRef.value.querySelector('#blinking-eye-right');
    blinkingEyeLeft = svgRef.value.querySelector('#blinking-eye-left');
  }
...
<template>
  <svg ref="svgRef" viewBox="0 0 1024 1024" class="w-full h-auto max-w-[50px]">
    <!-- Full hedgehog path -->
    <path
      fill="#191b27"
      class="dark:fill-[#f3f1ef]"
